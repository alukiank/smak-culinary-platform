<div align="center">
  ...
  <h1>SMAK</h1>
  <p><b>Кулінарна платформа на базі штучного інтелекту із семантичним пошуком рецептів та інтерактивним ШІ-шефом.</b></p>
  ...
  <p align="center">
    <img src="docs/smak-banner-ua.png" width="900" alt="Банер платформи SMAK" />
  </p>
...
<!-- Замініть плейсхолдери відео/GIF на реальні демонстраційні записи -->

### 1. Каталог рецептів, семантичний пошук та розширена фільтрація
Інтерактивна стрічка рецептів із комплексною панеллю фільтрації, природномовним пошуком та адаптивною сіткою страв.

<!-- Demo Video / GIF: Каталог рецептів, фільтри та гортання стрічки -->
...
Інтерактивний кулінарний асистент на базі Google Gemini (`gemini-3.1-flash-lite`), що підтримує багатокроковий виклик інструментів (tool calling) та потокову передачу відповідей через Server-Sent Events (SSE).

<!-- Demo Video / GIF: Взаємодія з ШІ-асистентом та виклик інструментів -->
...
**Огляд підписок:**
- **Автоматизований білінг та контроль квот:** вбудований віджет оплати LiqPay, відстеження статусу підписки, графік автоподовження та обмеження запитів на рівні Redis.

## Ключові можливості

- **Кулінарний ШІ-асистент**: діалоговий помічник на базі Google Gemini (`gemini-3.1-flash-lite`) із підтримкою багатокрокового виклику інструментів (пошук рецептів, дієтичні фільтри) та потокового стрімінгу через Server-Sent Events (SSE) з інтерактивними картками.
