...
<template>
  <div ref="fabContainerRef" class="fixed bottom-6 right-6 sm:bottom-8 sm:right-8 xl:right-[calc((100vw-80rem)/2+1.5rem)] z-[9999] select-none">
    ...
    <!-- Backdrop under the modal (mobile only, below header) -->
    <Transition name="fade">
      <div 
        v-if="isOpen"
        class="fixed top-16 inset-x-0 bottom-0 z-40 bg-white dark:bg-smak-neutral-950 sm:hidden cursor-pointer"
        aria-hidden="true"
        @click="isOpen = false"
      />
    </Transition>
    ...
    <!-- Universal Floating FAB Button (Always visible on mobile & desktop) -->
    <button
      @click.stop="toggleChat"
      type="button"
      class="relative z-50 group w-14 h-14 sm:w-15 sm:h-15 rounded-full bg-linear-to-tr from-purple-600 via-indigo-600 to-violet-500 text-white shadow-2xl shadow-purple-500/30 hover:shadow-purple-500/50 hover:scale-105 active:scale-95 flex items-center justify-center cursor-pointer transition-all duration-300 border-0 focus:outline-none"
